...
**Stale-proof replay (no circuit change).** `attest` discloses the roots it proves against, so they enter the transaction's public transcript. The ledger replays that transcript at apply time; if the baseline has moved, its own `checkRoot` query answers differently and consensus rejects the transaction. The CLI/service `replay` path exploits this deliberately — proving before a revocation and submitting after — to demonstrate revocation enforced by the chain rather than by the operator service. The transaction is held *unbalanced* until after the revocation, because a balanced transaction reserves the wallet's DUST.
...
6. **Four visible UI states. Three come from public ledger state; the red one is explicitly labelled as not.**
   - `COMPLIANT` — status COMPLIANT **and** `deviceEpoch == baselineEpoch` (green) — from the chain
   - `RE-ATTESTATION REQUIRED` — attested, but `deviceEpoch < baselineEpoch` (amber; every device enters this the instant the baseline changes) — from the chain
   - `NEVER ATTESTED` — no attestation has ever succeeded for this device — from the chain
   - `NON-COMPLIANT` (red) — the operator service's record of the last failed attempt, **not** ledger state. Two distinct causes, and the card distinguishes them via the attempt's `viaLedger` flag rather than by matching error text:
     - *local* — a required current-tree path no longer exists, so no proof and therefore no transaction could be built (the ordinary case);
     - *consensus* — a proof built before the revocation was submitted afterwards and the ledger refused the transaction (the `replay` path).
   Stronger and more honest than a bare green→red flip: after the CVE the clean device goes amber→green while the affected one goes amber→red, and the difference is produced by cryptography, not by UI state.
...
- 2026-08-25 (evening): **consensus-evidence stale-proof rejection implemented and verified on Preview.** A valid attestation is proven before the revocation, held *unbalanced* (a balanced transaction reserves the wallet's DUST and starves the revocation), then balanced and submitted after the roots move; the node refuses it (`1010: Invalid Transaction: Custom error: 104`). Circuits unchanged. Split-authority (regulator key for the component tree) deferred to post-hackathon.
- 2026-08-25 (evening): **deployed and the whole lifecycle executed on Preview** — contract `160c6bfc…ef6e`; firmware root verified byte-identical across a component revocation. Hashes in [docs/EVIDENCE.md](docs/EVIDENCE.md). Public read-only dashboard at https://nightseal.vercel.app.
- 2026-08-25 (evening): pinned `@midnight-ntwrk/onchain-runtime-v3` to a single version. Two copies (3.1.0 hoisted for `compact-runtime`'s `^3.0.0`, 3.0.0 nested for `midnight-js-protocol`) create two `StateValue` classes and break every live `callTx`, while deploys and simulator tests still pass.
